# BRG (BlackRice Go) 1.0 文件格式设计规范 (Draft)

## 一、 设计定位

**SGF/GIB 负责记录棋谱，BRG 负责记录棋评/研究成果。**

BRG (BlackRice Go) 不是为了取代 SGF/GIB 棋谱格式，而是建立在其之上，为“围棋研究文档/棋评”提供一种结构化、可长期保存、易于编辑、渲染和导出的标准化 JSON 文档格式。

在 TensuGo 中，用户通过浏览棋谱、调用 AI 分析、添加局面标记与讲解，最终保存生成一个 `.brg` 文件。
...
- **source**: 原始棋谱备份，包括原始格式 (`sgf` 或 `gib`)、文件名以及棋谱的完整文本内容（使 BRG 文件独立于外部棋谱文件，支持随时重载）。
...
BRG 1.0 的内容组织是线性的 Block 列表，这降低了 Notion 式编辑器的实现门槛。支持以下五种核心 Block。
...
```typescript
export interface BRGDocument {
  format: "brg";
  version: "1.0";
  meta: BRGMeta;
  source: BRGSource;
  gameInfo: BRGGameInfo;
  blocks: BRGBlock[];
}

export interface BRGMeta {
  title: string;
  author: string;
  createdAt: string;
  updatedAt: string;
...
export interface BRGSource {
  format: "sgf" | "gib";
  fileName: string;
  content: string; // 完整棋谱文本
}

export interface BRGGameInfo {
  boardSize: number;
  rules: string;
  komi: number;
  players: {
...
BRG 1.0 的设计保留了向 2.0 迁移的最佳兼容性路径：
...
4. **资源文件整合**：2.0 在引入图片、多媒体附件后，可通过顶层 `assets: []` 声明引用。同时，`.brg` 可以从单 JSON 升级为类似 EPUB 的 Zip 包压缩格式（包含 `.brg.json` 主文档与 `assets/` 资源目录），对 1.0 文件只读兼容。
