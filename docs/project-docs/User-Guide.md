...
- **出题与做题**：从复盘局面（AI 分析后）创建题目，支持 A/B 两种题型与自动出题标记；按目标级别做题，做题结果支持排名。
- **拟人引擎**：引擎配置可同时保存普通（Normal）与拟人（Human）两套 Model/Config，拟人引擎的候选数、水平偏移可调。
- **研究文档**：棋盘截图裁剪、变化图/PV 研究、PDF 与打包 HTML 导出、独立预览窗口。
- **批量分析**：批量分析控制台，实时查看日志与进度，可暂停/继续。
- **引擎内存监控**：分析时显示引擎内存占用。
...
进入 `设置 > 引擎` 后，可以看到引擎列表和当前选中配置。每个配置可以同时保存普通（Normal）与拟人（Human）两套 Model/Config，分析时按需要切换。
...
TensuGo 启动时不会自动扫描系统路径。只有点击 `Auto Detect` 才会扫描。分析时面板会显示引擎内存占用（V0.7.35 起），便于发现长时间分析导致的内存失控。
...
macOS 用内存压缩与交换来管理内存：应用释放掉的内存不会立即标记为空闲，而是先保留在压缩/缓存页里，等系统内存有压力时才逐步回收。这是 macOS 的原生 bug，不是 TensuGo 的疏忽——TensuGo 已经正常释放内存，只是 macOS 不会立刻把占用归还给系统。TensuGo 的 AI 分析会一次性分配较多内存，所以这个“不立即归还”的效果更明显，活动监视器里看起来会一直占用很高。进程退出后内存才会被完整回收、占用立刻下降。

Mac 用户请用完就退出 TensuGo（Cmd+Q），不要人离开时长时间挂着 AI 分析。
